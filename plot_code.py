...
def hist_3percent(pred, target, save_dir, top_percent):
    ...
    for i in range(len(top_percent)):
        ...
        plt.legend(loc=2)
    plt.tight_layout()
    plt.savefig(save_dir+'HIST_3%/'+'first+plot.png', dpi=300)      
    plt.close()
    # 畫負的
    # plt.figure()
    # sns.histplot(data=pred_f, log_scale=False, kde=True,bins=100, label="pred_value_of_-case", color='b')#, ax=ax2)
    # plt.title('Predicting value of positive cases')
    # for i in range(len(top_percent)):
    #     color = ['k','b','r']
...
def threshold_points(pred, target, save_dir, top_percent):
    ...
    # print(N_ofpositive)
    print(np.min(N_ofpositive))
    print(np.max(N_ofpositive))
    # print(point_threshold)
    print(np.min(point_threshold))
    print(np.max(point_threshold))
